...
function activate(context) {
    ...
    const checkCurrentFileCommand = vscode.commands.registerCommand('errorAssistant.checkCurrentFile', async () => {
        ...
        const diagnostics = vscode.languages.getDiagnostics(uri).filter((diag) => diag.severity === vscode.DiagnosticSeverity.Error);
        ...
        const errorDetails = diagnostics
            ...
            .join('\n');
        vscode.window
            .showWarningMessage(`Errors detected in ${uri.fsPath}:\n${errorDetails}\nDo you want to solve them?`, 'Yes', 'No')
            .then((selection) => {
            if (selection === 'Yes') {
                callApiWithError({
                    filePath: uri.fsPath,
    ...
    const diagnosticListener = vscode.languages.onDidChangeDiagnostics((event) => {
        const diagnostics = event.uris
            .map((uri) => ({
            uri,
            diagnostics: vscode.languages.getDiagnostics(uri).filter((diag) => diag.severity === vscode.DiagnosticSeverity.Error),
        }))
...
async function callApiWithError(errorData) {
    ...
    const apiEndpoint = config.get('apiEndpoint', 'http://localhost:8082/extention');
